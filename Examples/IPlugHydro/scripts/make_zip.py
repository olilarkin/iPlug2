...
def  main():
  ...
  installer = "\installer\IPlugHydro Installer.exe"
   
  if demo:
    installer = "\installer\IPlugHydro Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugHydro manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugHydro-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugHydro-v" + FULLVERSIONSTR + "-win-demo.zip"
