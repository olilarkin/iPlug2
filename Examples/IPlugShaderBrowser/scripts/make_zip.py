...
def  main():
  ...
  installer = "\installer\IPlugShaderBrowser Installer.exe"
   
  if demo:
    installer = "\installer\IPlugShaderBrowser Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugShaderBrowser manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugShaderBrowser-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugShaderBrowser-v" + FULLVERSIONSTR + "-win-demo.zip"
