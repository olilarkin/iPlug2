...
def  main():
  ...
  installer = "\installer\IPlugRealityView Installer.exe"
   
  if demo:
    installer = "\installer\IPlugRealityView Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugRealityView manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugRealityView-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugRealityView-v" + FULLVERSIONSTR + "-win-demo.zip"
