...
def  main():
  ...
  installer = "\installer\IPlugCPPSketch Installer.exe"
   
  if demo:
    installer = "\installer\IPlugCPPSketch Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugCPPSketch manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugCPPSketch-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugCPPSketch-v" + FULLVERSIONSTR + "-win-demo.zip"
