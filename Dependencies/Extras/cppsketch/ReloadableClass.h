...
public:
  ...
  void precompileHeader(const string &headerToPrecompile) {
    ...
    string cmd = "g++ -std=c++17 -x c++-header -stdlib=libc++ -DIGRAPHICS_NANOVG -DIGRAPHICS_METAL"
...
private:
  ...
  string cc() {
    ...
    string cppFlags = "-DIGRAPHICS_NANOVG -DIGRAPHICS_METAL";

  #ifdef __APPLE__
    cppFlags += " -stdlib=libc++ ";
  #endif
    string cmd = "g++ -g -std=c++17 "+cppFlags+" "+prefixHeaderFlag+" -Wno-deprecated-declarations -c " + cppFile + " -o " + objFile + " "
          + includes;
